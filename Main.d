...
struct ArgParser {
	void delegate(const(char)[]) err;
	struct Reg {
		const(char)[] t;
		void delegate() a;
		void delegate(const(char)[]) b;
	}
	Reg[] reg;
	void bind(const(char)[] t, void delegate() a) {
		reg ~= Reg(t.dup, a, null);
	}
	void bind(const(char)[] t, void delegate(const(char)[]) b) {
...
int main(char[][] allArgs) {
	...
	globalParams.compilerName = Environment.get("D_COMPILER", "dmd".dup);
	...
	try {
		//profile!("main")({
			...
			auto parser = ArgParser((const(char)[] arg) {
				throw new Exception("unknown argument: " ~ arg.idup);
			});
			...
			inout(char)[] olde(inout(char)[] arg) {
			...
			parser.bind("c",		(const(char)[] arg)	{ globalParams.compilerName = olde(arg); });
			parser.bind("C",		(const(char)[] arg)	{ globalParams.objExt = olde(arg); });		// HACK: should use profiles/configs instead
			parser.bind("O",		(const(char)[] arg)	{ globalParams.objPath = olde(arg); });
			parser.bind("D",		(const(char)[] arg)	{ globalParams.depsPath = olde(arg); });
			parser.bind("o",		(const(char)[] arg)	{ globalParams.outputFile = olde(arg); });
			parser.bind("x",		(const(char)[] arg)	{ globalParams.ignore ~= olde(arg); });
			parser.bind("modLimit",	(const(char)[] arg)	{ globalParams.maxModulesToCompile = cast(int)Integer.toInt(olde(arg)); });
			parser.bind("mod-limit=",	(const(char)[] arg){ globalParams.maxModulesToCompile = cast(int)Integer.toInt(arg); });
			...
			parser.bind("threads",	(const(char)[] arg)	{ globalParams.threadsToUse = cast(int)Integer.toInt(olde(arg)); });
			parser.bind("no-affinity",				{ globalParams.manageAffinity = false; });
			parser.bind("linker-affinity=",	(const(char)[] arg){ globalParams.linkerAffinityMask = cast(size_t)Integer.toInt(olde(arg), 16); });
			...
			parser.parse(args);
			        ...
			        //------------------------------------------------------------
                    void _ScanForModules (FilePath[] paths, ref const(char)[][] modules, bool recursive = false, bool justCheckAFolder = false) {
                            foreach (child; paths) {
                                    if (child.exists()) {
                                            if (!child.isFolder()) {
                                                    const(char)[] filename = child.file();
			...
			/+{
				if (Path.exists(globalParams.projectFile) && Path.isFile(globalParams.projectFile)) {
					...
					auto jobj = json.parse('{' ~ cast(const(char)[])File.get(globalParams.projectFile) ~ '}').toObject.hashmap();
